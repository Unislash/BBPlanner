                ...
                togglePerk: (perkId: string) =>
                    set((state) => {
                        ...
                        if (state.activePerkIds.indexOf(perkId) > -1) {
                            ...
                            if (
                                perkId === "student" ||
                                getAvailableNumberOfPerks(state.activePerkIds.length, maxLevel, state.isStudent) >= 1
                            ) {
...
export const useCanAddPerk = () =>
    useStore(
        perkStore,
        (state) => getAvailableNumberOfPerks(state.activePerkIds.length, maxLevel, state.isStudent) >= 1,
    );
