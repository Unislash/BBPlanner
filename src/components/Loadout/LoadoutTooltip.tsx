...
const HelmetContent: React.FC<{ item: Helmet }> = ({ item }) => (
    <>
        ...
        <StatLine>
            <StatIcon src={armor_head} />
            <b>{item.conditionMax}</b> &nbsp;maximum durability
        </StatLine>
        {item.staminaModifier && (
            <StatLine>
                <StatIcon src={fatigue} />
                Maximum Fatigue&nbsp;<b>{item.staminaModifier}</b>
            </StatLine>
        )}
    </>
);
...
const WeaponContent: React.FC<{ item: Weapon }> = ({ item }) => (
    <>
        ...
        {item.conditionMax && (
            <StatLine>
                <StatIcon src={asset_supplies} />
                <b>{item.conditionMax}</b> &nbsp;maximum durability
            </StatLine>
        )}
        <StatLine>
            <StatIcon src={regular_damage} />
            Damage of&nbsp;
            <b>
                {item.regularDamage} - {item.regularDamageMax}
            </b>
        </StatLine>
        <StatLine>
            <StatIcon src={direct_damage} />
            <b>{Math.round(parseFloat(item.directDamageMult) * 100)}%</b>&nbsp;of damage ignores armor
        </StatLine>
        <StatLine>
            <StatIcon src={armor_damage} />
            <b>{Math.round(parseFloat(item.armorDamageMult) * 100)}%</b>&nbsp;effective against armor
        </StatLine>
        {item.chanceToHitHead && (
            <StatLine>
                <StatIcon src={chance_to_hit_head} />
                Chance to hit head&nbsp;<b>+{item.chanceToHitHead}%</b>
            </StatLine>
        )}
        {item.rangeMax && (
            <StatLine>
                <StatIcon src={vision} />
                Range of&nbsp;<b>{item.rangeMax}</b>&nbsp;tiles
            </StatLine>
        )}
...
const OffhandContent: React.FC<{ item: Offhand }> = ({ item }) => (
    <>
        ...
        {"conditionMax" in item && (
            <StatLine>
                <StatIcon src={asset_supplies} />
                <b>{item.conditionMax}</b> &nbsp;maximum durability
            </StatLine>
        )}
        {"meleeDefense" in item && (
            <StatLine>
                <StatIcon src={melee_defense} />
                Melee Defense&nbsp;<b>+{item.meleeDefense}</b>
            </StatLine>
        )}
        {"rangedDefense" in item && (
            <StatLine>
                <StatIcon src={ranged_defense} />
                Ranged Defense&nbsp;<b>+{item.rangedDefense}</b>
            </StatLine>
        )}
        {item.staminaModifier && (
            <StatLine>
...
export const LoadoutTooltip: React.FC<LoadoutTooltipProps> = ({ loadoutSlotType, item }) => (
    <div>{switchOnSlotType(loadoutSlotType, item)}</div>
);
